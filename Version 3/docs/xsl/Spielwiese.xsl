...
>
	...
	"/>
	<xsl:variable name="schemaBasis"        select="$schemata/xs:schema[xs:annotation/xs:appinfo/api:schema/@name='Basis']" />
	<xsl:variable name="schemaTarifAngebot" select="$schemata/xs:schema[xs:annotation/xs:appinfo/api:schema[@name='Tarif' or @name='Angebot']]" />
<!-- aktuell sind Tarif und Angebot identisch mit Tarif/Angebot		 -->
<!-- 	<xsl:variable name="schemaTarif"        select="$schemata/xs:schema[xs:annotation/xs:appinfo/api:schema/@name='Tarif']" /> -->
<!-- 	<xsl:variable name="schemaAngebot"      select="$schemata/xs:schema[xs:annotation/xs:appinfo/api:schema/@name='Angebot']" /> -->
	<xsl:variable name="schemaVertrieb"     select="$schemata/xs:schema[xs:annotation/xs:appinfo/api:schema/@name='Vertrieb']" />
	<xsl:variable name="schemaErgebnis"     select="$schemata/xs:schema[xs:annotation/xs:appinfo/api:schema/@name='Ergebnis']" />
	...
	<xsl:template name="windows-pfad-zu-uri">
<!-- 		<xsl:variable name="def" select="$defaults" />		 -->
		<xsl:variable name="def" select="if ( ends-with($defaults, '\')) then substring($defaults, 1, string-length($defaults)-1) else $defaults " />		
...
<!-- 		<xsl:variable name="path"   select="concat('', $def)"/> -->
		...
		<xsl:value-of select="api:out(concat('defaults=', $defaults))"/>
	...
	<xsl:template name="schema-elemente">
		<xsl:value-of select="concat('- ',string-join($schemata/xs:schema/xs:annotation/xs:appinfo/api:schema/@name, concat($crlf, '- ')))"/>
		<xsl:value-of select="$crlf"/>
		<xsl:value-of select="$crlf"/>
<!-- 		<xsl:value-of select="string-join($schemata/xs:schema[xs:annotation/xs:appinfo/api:schema/@name='Basis']/xs:element/@name, $crlf)"/> -->
		<xsl:value-of select="concat('## Basiselemente:', $crlf, $crlf, '### B_', string-join($schemaBasis/xs:element/@name, concat($crlf, $crlf, '### B_')))"/>
		<xsl:value-of select="$crlf"/>
		<xsl:value-of select="$crlf"/>
		<xsl:value-of select="concat('## Tarif/Angebot:', $crlf, $crlf, '### TA_', string-join($schemaTarifAngebot/xs:element/@name, concat($crlf, $crlf, '### TA_')))"/>
<!-- aktuell sind Tarif und Angebot identisch mit Tarif/Angebot		 -->
<!-- 		<xsl:value-of select="$crlf"/> -->
<!-- 		<xsl:value-of select="$crlf"/> -->
<!-- 		<xsl:value-of select="concat('## Tarif:',         $crlf, $crlf, '### T_', string-join($schemaTarif/xs:element/@name, concat($crlf, $crlf, '### T_')))"/> -->
<!-- 		<xsl:value-of select="$crlf"/> -->
<!-- 		<xsl:value-of select="$crlf"/> -->
<!-- 		<xsl:value-of select="concat('## Angebot:',       $crlf, $crlf, '### A_', string-join($schemaAngebot/xs:element/@name, concat($crlf, $crlf, '### A_')))"/> -->
		<xsl:value-of select="$crlf"/>
		<xsl:value-of select="$crlf"/>
		<xsl:value-of select="concat('## Vertrieb:',      $crlf, $crlf, '### V_', string-join($schemaVertrieb/xs:element/@name, concat($crlf, $crlf, '### V_')))"/>
		<xsl:value-of select="$crlf"/>
		<xsl:value-of select="$crlf"/>
		<xsl:value-of select="concat('## Ergebnisdaten:', $crlf, $crlf, '### E_', string-join($schemaErgebnis/xs:element/@name, concat($crlf, $crlf, '### E_')))"/>
...
# xs:schema
		...
		<xsl:call-template name="schema-elemente" />
<!-- 		<xsl:call-template name="windows-pfad-zu-uri" /> -->
