...
>
  ...
  <!-- ===============================================================================
    ...
    Erzeugt ein SQL Script um eine leere HUSST_Versorgungsdaten_3_0_0 Datenbank 
    zu erstellen.
    ...
    <xsl:if test="not(@type)">
      <xsl:choose>
        ...
        <xsl:when test="xs:restriction/@base">
          <xsl:call-template name="normalizeDBObjectName">
            <xsl:with-param name="name" select="xs:restriction/@base"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template mode="fieldtype" match="*">
    ...
    <xsl:variable name="type">
      <xsl:apply-templates mode="fieldtypename" select="."/>
    </xsl:variable>
    ...
    <xsl:choose>
      ...
      <xsl:when test="($type='string') or ($type='language')">
        <xsl:if test="$maxLength">varchar(<xsl:value-of select="$maxLength"></xsl:value-of>)
        </xsl:if>
  ...
  <xsl:template match="/xs:schema/xs:element" mode="tabledef">
    ...
    <xsl:if test="not(/xs:schema/xs:*[@name=$typeName])">
      <xsl:message>
        <xsl:text>FEHLER: Datentyp</xsl:text>
        <xsl:value-of select="@type"/>
        <xsl:text>ist nicht definiert.</xsl:text>
      </xsl:message>
    </xsl:if>
  ...
  <xsl:template match="/xs:schema/xs:complexType" mode="tabledef">
    ...
    <xsl:variable name="tableName">
      <xsl:call-template name="normalizeDBObjectName">
        <xsl:with-param name="name" select="substring-before(@name,'_Type')"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="typeName">
      <xsl:call-template name="normalizeDBObjectName">
  ...
  <xsl:template match="api:uniquekey" mode="tabledef">
    ...
    <xsl:variable name="typeName">
      <xsl:call-template name="normalizeDBObjectName">
        <xsl:with-param name="name" select="ancestor::xs:*/@name"/>
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template match="/xs:schema">
    ...
    <xsl:text>/************************************************************************************</xsl:text>
    <xsl:value-of select="$crlf"/>
    <xsl:text>* HUSST Versorgungsdaten</xsl:text>
    <xsl:value-of select="$crlf"/>
    <xsl:text>*</xsl:text>
    <xsl:value-of select="$crlf"/>
    ...
    <xsl:text>*</xsl:text>
    <xsl:value-of select="$crlf"/>
    <xsl:text>* HUSST 3_00 (</xsl:text>
    <xsl:value-of select="$HUSSTversion"/>
    <xsl:text>)</xsl:text>
    <xsl:value-of select="$crlf"/>
    <xsl:text>*</xsl:text>
    <xsl:value-of select="$crlf"/>
